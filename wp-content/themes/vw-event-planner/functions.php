...
function vw_event_planner_scripts() {
	...
	wp_enqueue_style( 'vw-event-planner-block-style', get_theme_file_uri('/assets/css/blocks.css') );
	wp_enqueue_style( 'bootstrap-style', esc_url(get_template_directory_uri()).'/assets/css/bootstrap.css' );
	wp_enqueue_style( 'vw-event-planner-block-patterns-style-frontend', get_theme_file_uri('/inc/block-patterns/css/block-frontend.css') );
	...
	wp_enqueue_script( 'vw-event-planner-custom-scripts-jquery', esc_url(get_template_directory_uri()) . '/assets/js/custom.js', array('jquery') );
 ...
 */
function vw_event_planner_block_editor_styles() {
	wp_enqueue_style( 'vw-event-planner-font', vw_event_planner_font_url(), array() );
	wp_enqueue_style( 'bootstrap-style', esc_url(get_template_directory_uri()).'/assets/css/bootstrap.css' );
	wp_enqueue_style( 'font-awesome-css', esc_url(get_template_directory_uri()).'/assets/css/fontawesome-all.css' );
    wp_enqueue_style( 'vw-event-planner-block-patterns-style-editor', get_theme_file_uri( '/inc/block-patterns/css/block-editor.css' ), false, '1.0', 'all' );
}
add_action( 'enqueue_block_editor_assets', 'vw_event_planner_block_editor_styles' );
...
function vw_event_planner_sanitize_float( $input ) {
	return filter_var($input, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
}

function vw_event_planner_sanitize_number_range( $number, $setting ) {
...
function vw_event_planner_sanitize_phone_number( $phone ) {
...
add_filter('loop_shop_columns', 'vw_event_planner_loop_columns');
	...
	function vw_event_planner_loop_columns() {
		return get_theme_mod( 'vw_event_planner_products_per_row', '3' ); 
		// 3 products per row
	}
}

//Change number of products that are displayed per page (shop page)
add_filter( 'loop_shop_per_page', 'vw_event_planner_products_per_page' );
function vw_event_planner_products_per_page( $cols ) {
  	return  get_theme_mod( 'vw_event_planner_products_per_page',9);
}

function vw_event_planner_logo_title_hide_show(){
	if(get_theme_mod('vw_event_planner_logo_title_hide_show') == '1' ) {
		return true;
	}
	return false;
}

function vw_event_planner_tagline_hide_show(){
	if(get_theme_mod('vw_event_planner_tagline_hide_show') == '1' ) {
		return true;
	}
	return false;
}
