...
class Ibtana_Visual_Editor_Menu_Creator extends Ibtana_Visual_Editor_Menu_Class {
	...
	public function ibtana_visual_editor_ive_addons_page() {
		...
		<div class="wrap">
			...
			<div class="wp-list-table widefat ive-addon-card">
						  ...
						  <div class="plugin-card-top">
						    <div class="name column-name">
						      <h3>
						        ...
						        <a href="<?php echo esc_url($plugin->permalink); ?>" target="_blank" class="plugin-name">
						          <img src="<?php if( isset($plugin->image) && ( $plugin->image != '' ) ) { echo esc_url( $plugin->image ); } else { echo IBTANA_PLUGIN_URI . 'dist/images/ibtana-setting-icon.svg'; } ?>" class="plugin-icon">
						        </a>
						      </h3>
						    </div>
						    <div class="action-links">
									<a target="_blank" href="<?php echo esc_url($plugin->permalink); ?>" type="button" class="button">
										<?php if ( property_exists( $plugin, 'price' ) && ( $plugin->price != '' ) ): ?>
											<?php esc_html_e( '$' . $plugin->price . ' Buy Now!', 'ibtana-visual-editor' ); ?>
										<?php else: ?>
